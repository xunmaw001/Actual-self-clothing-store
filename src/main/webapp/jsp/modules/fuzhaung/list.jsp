...
<body>
    <div class="modal fade" id="plusOrReduceFuzhaungKucunNumberModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <%-- 模态框标题--%>
                <div class="modal-header">
                    <h5 class="modal-title" id="plusOrReduceFuzhaungKucunNumberTitle">增加或减少库存</h5>
                ...
                <div class="modal-body">
                    数量:<input type="number" id="plusOrReduceFuzhaungKucunNumber" style="width: 300px;" class="form-control form-control-sm"
                    ...
                    <input type="hidden" id="fuzhaungId">
                    <input type="hidden" id="hideflag">
                    <input type="hidden" id="thisFuzhaungKucunNumber">
                </div>
                <%-- 模态框按钮 --%>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                    <button type="button" onclick="submitPlusOrReduceFuzhaungKucunNumber()" class="btn btn-primary">提交</button>
                </div>
            </div>
        </div>
    </div>
...
<!--/Pre Loader -->
                                ...
                                <div class="col-sm-12">
                                    ...
                                    <div class="layui-inline" style="margin-bottom: 10px;">
                                        ...
                                        <div class="layui-input-inline">
                                            <input type="text" id="fuzhaungUuidNumberSearch" style="width: 140px;" class="form-control form-control-sm"
    ...
    <!-- 本表的查询条件 -->
        ...
        function setDataRow(item, number) {
            ...
            //服装编号
            var fuzhaungUuidNumberCell = document.createElement('td');
            fuzhaungUuidNumberCell.innerHTML = item.fuzhaungUuidNumber;
            row.appendChild(fuzhaungUuidNumberCell);
            ...
            //服装库存
            var fuzhaungKucunNumberCell = document.createElement('td');
            fuzhaungKucunNumberCell.innerHTML = item.fuzhaungKucunNumber;
            row.appendChild(fuzhaungKucunNumberCell);


            //购买获得积分
            var fuzhaungPriceCell = document.createElement('td');
            fuzhaungPriceCell.innerHTML = item.fuzhaungPrice;
            row.appendChild(fuzhaungPriceCell);


            //服装原价
            var fuzhaungOldMoneyCell = document.createElement('td');
            fuzhaungOldMoneyCell.innerHTML = item.fuzhaungOldMoney;
            row.appendChild(fuzhaungOldMoneyCell);


            //现价/积分
            var fuzhaungNewMoneyCell = document.createElement('td');
            fuzhaungNewMoneyCell.innerHTML = item.fuzhaungNewMoney;
            row.appendChild(fuzhaungNewMoneyCell);


            //点击次数
            var fuzhaungClicknumCell = document.createElement('td');
            fuzhaungClicknumCell.innerHTML = item.fuzhaungClicknum;
            row.appendChild(fuzhaungClicknumCell);
            ...
            var plusAttr = "plusFuzhaungKucunNumber(" + item.id+','+item.fuzhaungKucunNumber+")";
            ...
            var reduceAttr = "reduceFuzhaungKucunNumber(" + item.id+','+item.fuzhaungKucunNumber+')';
        ...
        function remove(id) {
            ...
            if (mymessage == true) {
                ...
                httpJson("fuzhaung/delete", "POST", paramArray, (res) => {
                    if(res.code == 0){
                        getDataList();
                        alert('删除成功');
                    }
        ...
        //添加
        function add() {
            window.sessionStorage.setItem("addfuzhaung", "addfuzhaung");
            window.location.href = "add-or-update.jsp"
        }
        ...
        function shangxia(id,shangxiaTypes) {
            ...
            httpJson("fuzhaung/update", "POST", data, (res) => {
                if(res.code == 0){
                    alert(msg);
                    getDataList()
                }else{
    ...
    //查询当前表搜索条件所有列表
        ...
        //增加库存
        function plusFuzhaungKucunNumber(thisId,fuzhaungKucunNumber) {
            if(thisId == null || thisId=="" || thisId=="null"){
                alert("未知错误,请联系管理员处理");
                return;
            }
            $("#plusOrReduceFuzhaungKucunNumberTitle").val("增加库存");
            $("#thisFuzhaungKucunNumber").val(fuzhaungKucunNumber);
            $("#hideflag").val("+");
            $("#fuzhaungId").val(thisId);
            id=thisId;
            $('#plusOrReduceFuzhaungKucunNumberModal').modal('show');
        }

        //减少库存
        function reduceFuzhaungKucunNumber(thisId,fuzhaungKucunNumber) {
            if(thisId == null || thisId=="" || thisId=="null"){
                alert("未知错误,请联系管理员处理");
                return;
            }
            $("#plusOrReduceFuzhaungKucunNumberTitle").val("减少库存");
            $("#thisFuzhaungKucunNumber").val(fuzhaungKucunNumber);
            $("#hideflag").val("-");
            $("#fuzhaungId").val(thisId);
            id=thisId;
        ...
        $('#plusOrReduceFuzhaungKucunNumberModal').on('show.bs.modal', function () {
            //清理表格内容,防止重复回显
            $("#plusOrReduceFuzhaungKucunNumber").val(0);
            if(id ==null){
                alert("获取当前列id错误,请联系管理员处理");
                $('#plusOrReduceFuzhaungKucunNumberModal').modal('hide');
                $("#plusOrReduceFuzhaungKucunNumberTitle").val("");
                $("#thisFuzhaungKucunNumber").val("");
                $("#hideflag").val("");
                $("#fuzhaungId").val("");
                getDataList();
                return;
        ...
        //模态框关闭的时候会执行这个里面的东西
        $('#plusOrReduceFuzhaungKucunNumberModal').on('hide.bs.modal', function () {
            id=null;
            $("#plusOrReduceFuzhaungKucunNumber").val(0);
            $("#plusOrReduceFuzhaungKucunNumberTitle").val("");
            $("#thisFuzhaungKucunNumber").val("");
        ...
        function submitPlusOrReduceFuzhaungKucunNumber() {
            var id = $("#fuzhaungId").val();//当前数据id
            var hideflag = $("#hideflag").val();//是加还是减
            var thisFuzhaungKucunNumber = $("#thisFuzhaungKucunNumber").val();//原先的数量
            var plusOrReduceFuzhaungKucunNumber = $("#plusOrReduceFuzhaungKucunNumber").val();//增加或者减少的数量
            var sumNumber=0;
            var msg;
            if(hideflag == "+"){
                sumNumber =parseInt(thisFuzhaungKucunNumber) + parseInt(plusOrReduceFuzhaungKucunNumber);
                msg = "增加库存成功";
            }else if(hideflag == "-"){
                sumNumber =parseInt(thisFuzhaungKucunNumber) - parseInt(plusOrReduceFuzhaungKucunNumber);
                msg = "减少库存成功";
            ...
            let data = {"id":id,"fuzhaungKucunNumber":sumNumber};
            httpJson("fuzhaung/update", "POST", data, (res) => {
                if(res.code == 0){
                    alert(msg);
                    getDataList();
