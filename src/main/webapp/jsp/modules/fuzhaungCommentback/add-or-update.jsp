                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    <div class="form-group col-md-6 fuzhaungCommentbackTextDiv">
                                        <label>评价内容</label>
                                        <textarea style="width: 450px"  id="fuzhaungCommentbackText" name="fuzhaungCommentbackText" class="form-control"placeholder="评价内容"></textarea>
                                    </div>
...
<script>
    ...
    var tableName = "fuzhaungCommentback";
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("fuzhaungCommentback/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addfuzhaungCommentback');
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                fuzhaungId: "required",
                yonghuId: "required",
                fuzhaungCommentbackText: "required",
                replyText: "required",
            },
            messages: {
                fuzhaungId: "服装不能为空",
                yonghuId: "用户不能为空",
                fuzhaungCommentbackText: "评价内容不能为空",
                replyText: "回复内容不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addfuzhaungCommentback = window.sessionStorage.getItem("addfuzhaungCommentback");
        if (addfuzhaungCommentback != null && addfuzhaungCommentback != "" && addfuzhaungCommentback != "null") {
            //注册表单验证
            $(validform());
            ...
            window.sessionStorage.removeItem('updateId');
            http("fuzhaungCommentback/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#fuzhaungCommentbackText").val(ruleForm.fuzhaungCommentbackText);
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            $('#fuzhaungCommentbackText').attr('readonly', 'readonly');
