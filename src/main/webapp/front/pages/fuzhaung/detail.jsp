        ...
        <div class="data-detail-breadcrumb"
             style="width: 1110px; padding: 0px 10px; box-shadow: rgba(255, 0, 0, 0.3) 0px 0px 0px; margin: 20px auto; border-color: rgb(135, 206, 250); border-radius: 4px; border-width: 0px; border-style: solid; height: 54px;">
                <span class="layui-breadcrumb">
                    ...
                    <a><cite>{{detail.fuzhaungName}}</cite></a>
                </span>
            ...
            <div class="row">
                <div class="col-lg-8">
                    <div class="blog-details-wrapper">
                        <div><img :src="detail.fuzhaungPhoto" alt="image" style="width: 80%;height: auto"></div>
                        <div class="blog-details-header">
                            <h3 class="blog-details-title">{{title}}</h3>
                            <ul class="post-meta">
                                <li v-if="detail.fuzhaungUuidNumber">服装编号：
                                    {{detail.fuzhaungUuidNumber}}
                                </li>
                                <li v-if="detail.fuzhaungTypes">服装类型：
                                    {{detail.fuzhaungValue}}
                                </li>
                                <li v-if="detail.fuzhaungKucunNumber">服装库存：
                                    {{detail.fuzhaungKucunNumber}}
                                </li>
                                <li v-if="detail.fuzhaungPrice">购买获得积分：
                                    {{detail.fuzhaungPrice}}
                                </li>
                                <li v-if="detail.fuzhaungOldMoney">服装原价：
                                    {{detail.fuzhaungOldMoney}}
                                </li>
                                <li v-if="detail.fuzhaungNewMoney">现价/积分：
                                    {{detail.fuzhaungNewMoney}}
                                </li>
                                <li v-if="detail.fuzhaungClicknum">点击次数：
                                    {{detail.fuzhaungClicknum}}
                                </li>
                            </ul>
                            ...
                            <div style="padding: 20px;border: 1px solid #f2f2f2;margin-top: 20px;display: flex">
                                <div class="num-picker">
                                    ...
                                    <button onclick="addFuzhaungCart()" style="height:auto;"
                                    ...
                                    <button onclick="addFuzhaungOrder()" style="height:auto;"
                        ...
                        <div class="blog-details-content" v-html="myFilters(detail.fuzhaungContent)">
                        </div>
                    </div>
                    <div class="comments-area" v-if="fuzhaungCommentbackDataList.length > 0">
                        <h3 class="title">
                            评论
                        </h3>
                        <ul class="comments-list">
                            <li class="single-comment" v-for="(item,index) in fuzhaungCommentbackDataList"
                                v-bind:key="index">
                                <div class="thumb">
                                ...
                                <div class="content">
                                    ...
                                    <p>{{item.fuzhaungCommentbackText}}</p>
                                </div>
                ...
                <div class="col-lg-4" v-if="fuzhaungRecommendList">
                    <div class="sidebar">
                        ...
                        <div class="widget">
                            ...
                            <ul class="small-post-list">
                                <li class="small-post-single" v-for="(item,index) in fuzhaungRecommendList"
                                    v-bind:key="index" @click="jump('../fuzhaung/detail.jsp?id='+item.id)">
                                    <div class="thumb"><img width="65px" :src="item.fuzhaungPhoto" alt="image"></div>
                                    <div class="content">
                                        <a class="main_color" class="date">{{item.fuzhaungName}}</a>
                                        <h6 v-if="item.fuzhaungNewMoney" class="post-title"><a style="color: red">{{item.fuzhaungNewMoney}}</a>RMB
                                        </h6>
                                    </div>
                                </li>
                            </ul>
...
<script>
    ...
    var vue = new Vue({
        ...
        data: {
            ...
            //系统推荐
            fuzhaungRecommendList: [],
            dataList: [],
            // 加入购物车数量
            buyNumber: 1,
            // 当前详情页表
            detailTable: 'fuzhaung',
            // 评价列表
            fuzhaungCommentbackDataList: [],
        },
        //  清除定时器
        destroyed: function () {
            // 不知道具体作用
        ...
        methods: {
            ...
            , fuzhaungYuyue() {
                let _this = this;
                // localStorage.setItem('fuzhaungId', _this.detail.id);
                // _this.jump("../fuzhaungOrder/add.jsp");

                let data = {
                    fuzhaungId: _this.detail.id,
                    yonghuId: localStorage.getItem("userid"),
                    fuzhaungOrderYesnoTypes: 1,
                }
                // 提交数据
                layui.http.requestJson('fuzhaungOrder/add', 'post', data, function (res) {
                    if (res.code == 0) {
                        layui.layer.msg('预约成功', {
                            time: 2000,
                            icon: 6
                        }, function () {
                            _this.jump("../fuzhaungOrder/list.jsp");
                        });
                    } else {
    ...
    layui.use(['layer', 'form', 'element', 'carousel', 'http', 'jquery', 'laypage', 'rate'], function () {
        ...
        http.request(`${vue.detailTable}/detail/` + id, 'get', {}, function (res) {
            // 详情信息
            vue.detail = res.data;
            vue.title = vue.detail.fuzhaungName;
            // 轮播图片
            vue.swiperList = vue.detail.fuzhaungPhoto ? vue.detail.fuzhaungPhoto.split(",") : [];
            var swiperItemHtml = '';
        ...
        // 系统推荐
        http.request(`fuzhaung/list`, 'get', {
            page: 1,
            limit: 5,
            fuzhaungTypes: vue.detail.fuzhaungTypes,
            fuzhaungDelete: 1,
            ...
            vue.fuzhaungRecommendList = res.data.list;
        });
        ...
        http.request(`${vue.detailTable}Commentback/list`, 'get', {
            page: 1,
            limit: limit,
            fuzhaungId: vue.detail.id
        }, function (res) {
            vue.fuzhaungCommentbackDataList = res.data.list;
            // 分页
            laypage.render({
                elem: 'fuzhaungCommentbackPager',
    ...
    // 添加到购物车
    function addFuzhaungCart() {
        if (vue.detail.fuzhaungKucunNumber > 0 && vue.detail.fuzhaungKucunNumber < vue.buyNumber) {
            layer.msg(`库存不足`, {
                time: 2000,
                icon: 5
            });
    ...
    function addFuzhaungOrder() {
        ...
        localStorage.setItem('fuzhaungs', JSON.stringify([{
            fuzhaungId: vue.detail.id,
            fuzhaungUuidNumber: vue.detail.fuzhaungUuidNumber,
            fuzhaungName: vue.detail.fuzhaungName,
            fuzhaungPhoto: vue.detail.fuzhaungPhoto,
            fuzhaungTypes: vue.detail.fuzhaungTypes,
            fuzhaungKucunNumber: vue.detail.fuzhaungKucunNumber,
            fuzhaungPrice: vue.detail.fuzhaungPrice,
            fuzhaungOldMoney: vue.detail.fuzhaungOldMoney,
            fuzhaungNewMoney: vue.detail.fuzhaungNewMoney,
            fuzhaungClicknum: vue.detail.fuzhaungClicknum,
            fuzhaungContent: vue.detail.fuzhaungContent,
            shangxiaTypes: vue.detail.shangxiaTypes,
            fuzhaungDelete: vue.detail.fuzhaungDelete,
            createTime: vue.detail.createTime,
            buyNumber: vue.buyNumber,
            yonghuId: localStorage.getItem('userid'),
        }]));
        // 跳转到确认下单页面
        jump('../fuzhaungOrder/confirm.jsp');
    }
