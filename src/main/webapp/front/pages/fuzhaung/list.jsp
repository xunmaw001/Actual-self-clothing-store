                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="fuzhaungUuidNumber" id="fuzhaungUuidNumber" placeholder="服装编号" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="fuzhaungName" id="fuzhaungName" placeholder="服装名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <select name="fuzhaungTypes" id="fuzhaungTypes" lay-filter="fuzhaungTypes">
                    ...
                    <div class="show-reel">
                        <div v-for="(item,index) in dataList" v-bind:key="index" class="col-md-3 agile-gallery-grid">
                            <div class="agile-gallery">
                                <a @click="jumpCheck('../fuzhaung/detail.jsp?id='+item.id , item.aaaaaaaaaa == null?'':item.aaaaaaaaaa , item.shangxiaTypes == null?'':item.shangxiaTypes)" class="lsb-preview" data-lsb-group="header">
                                    <img :src="item.fuzhaungPhoto?item.fuzhaungPhoto.split(',')[0]:''" style="height: 350px;width:100%;" />
                                    <div class="agileits-caption">
                                        <h4>{{item.fuzhaungName}}</h4>
                                        <p v-if="item.fuzhaungNewMoney != null" >￥{{item.fuzhaungNewMoney}}</p>
                                    </div>
                                </a>
                            </div>
                        </div>
...
<script>
    ...
    // 当前表的字典表
        ...
        function fuzhaungTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=fuzhaung_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        function pageList() {
            ...
            if (jquery('#fuzhaungUuidNumber').val() != null && jquery('#fuzhaungUuidNumber').val() != "" && jquery('#fuzhaungUuidNumber').val() != "null" ) {
                param['fuzhaungUuidNumber'] = jquery('#fuzhaungUuidNumber').val();
            }
         
            if (jquery('#fuzhaungName').val() != null && jquery('#fuzhaungName').val() != "" && jquery('#fuzhaungName').val() != "null" ) {
                param['fuzhaungName'] = jquery('#fuzhaungName').val();
            }
                 
            var fuzhaungTypes = $("#fuzhaungTypes").find("option:selected").val();
            if (fuzhaungTypes != null && fuzhaungTypes != "" && fuzhaungTypes != "null") {
                param['fuzhaungTypes'] = fuzhaungTypes;
            }
                                                                        

            // 获取列表数据
            http.request('fuzhaung/list', 'get', param, function(res) {
