        ...
        <div class="cartBox">
            ...
            <div class="order_content">
                <ul class="order_lists" v-for="(item,index) in dataList" v-bind:key="index">
                    ...
                    <li class="list_con">
                        <div class="list_img"><img :src="item.fuzhaungPhoto"></div>
                        <div class="list_text">{{item.fuzhaungName}}</div>
                    </li>
                    <li class="list_price">
                        <p class="price">{{item.fuzhaungNewMoney}} RMB</p>
                    </li>
                    ...
                    <li class="list_sum">
                        <p class="sum_price">{{(item.fuzhaungNewMoney*item.buyNumber).toFixed(2)}} RMB</p>
                    </li>
                    <li class="list_op">
    ...
    var vue = new Vue({
        ...
        methods: {
			...
			,addCartNum(index) {
				...
				layui.http.request(`fuzhaung/info/${item.fuzhaungId}`, 'get', {}, function(res) {
					// 库存限制
					if (res.data.fuzhaungKucunNumber && res.data.fuzhaungKucunNumber > 0 && res.data.fuzhaungKucunNumber  <= item.buyNumber) {
			...
			// 购买
			buyClick() {
				localStorage.setItem('fuzhaungs', JSON.stringify(this.dataList));
				window.location.href = '../fuzhaungOrder/confirm.jsp'
			}
        }
    });
