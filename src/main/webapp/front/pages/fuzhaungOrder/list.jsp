        ...
        <div class="modal-dialog" style="z-index: 3000">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <textarea id="fuzhaungCommentbackContent" style="width: 500px;height: 300px"></textarea>
                </div>
                <%-- 模态框按钮 --%>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                    <button type="button" @click="submitFuzhaungCommentback()" class="btn btn-theme">评价</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="right-container" style="padding-top: 0;">
            <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
                <ul class="layui-tab-title">
                    ...
                    <li v-for="item in fuzhaungOrderTypesList" :value="item.codeIndex"  @click="pageList(item.codeIndex)">{{item.indexName}}</li>
                </ul>
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            ...
                            <td style="width: 60px;">{{item.fuzhaungName}}</td>
                            <td style="width: 250px;">
                                <img :src="item.fuzhaungPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            </td>
                            <td style="width: 80px;">{{item.fuzhaungValue}}</td>
                            <td style="width: 60px;">{{item.yonghuName}}</td>
                            <td style="width: 150px;">{{item.fuzhaungOrderUuidNumber}}</td>
                            <td style="width: 150px;">{{item.buyNumber}}</td>
                            <td style="width: 150px;">{{item.fuzhaungOrderTruePrice}}</td>
                            <td style="width: 150px;">{{item.fuzhaungOrderCourierName}}</td>
                            <td style="width: 150px;">{{item.fuzhaungOrderCourierNumber}}</td>
                            <td style="width: 80px;">{{item.fuzhaungOrderValue}}</td>
                            <td style="width: 80px;">{{item.fuzhaungOrderPaymentValue}}</td>
                            <td style="width: 150px;">{{item.insertTime}}</td>
                            <td style="width: 100px;">
                                <button v-if="item.fuzhaungOrderTypes==1" @click="refund(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                ...
                                <button v-if="item.fuzhaungOrderTypes==3" @click="receiving(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                ...
                                <button v-if="item.fuzhaungOrderTypes==4" @click="commentback(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius btn-theme">
...
<script>
    var vue = new Vue({
        ...
        methods: {
            ...
            //列表查询
            ,pageList(fuzhaungOrderTypes) {
                this.fuzhaungOrderTypes = fuzhaungOrderTypes;
                // 获取列表数据
                layui.http.request('fuzhaungOrder/page', 'get', {
                    page: 1,
                    limit: this.limit,
                    fuzhaungOrderTypes: (vue.fuzhaungOrderTypes == 0)?null:vue.fuzhaungOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('fuzhaungOrder/page', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    fuzhaungOrderTypes: (this.fuzhaungOrderTypes == 0)?null:this.fuzhaungOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`fuzhaungOrder/pay?id=`+id, 'GET',{}, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
                            time: 2000,
                            icon: 6
            ...
            ,refund(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`fuzhaungOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
            ...
            ,receiving(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`fuzhaungOrder/receiving?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('成功收货', {
                            time: 2000,
                            icon: 6
            ...
            ,submitFuzhaungCommentback(){
                var fuzhaungCommentbackContent = $("#fuzhaungCommentbackContent").val();
                var id = $("#id").val();

                layui.http.request("fuzhaungOrder/commentback?id="+id+"&commentbackText="+fuzhaungCommentbackContent, 'get', {}, function (res) {
                    if(res.code==0){
                        layer.msg('评价成功', {
                            time: 2000,
                            icon: 6
    ...
    // 当前表的字典表
        ...
        function fuzhaungOrderTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=fuzhaung_order_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        function fuzhaungOrderPaymentTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=fuzhaung_order_payment_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        // 获取列表数据
        http.request('fuzhaungOrder/page', 'get',{
            page: 1,
            limit: limit,
            fuzhaungOrderTypes: (vue.fuzhaungOrderTypes == 0)?null:vue.fuzhaungOrderTypes
        }, function(res) {
            ...
            laypage.render({
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('fuzhaungOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            fuzhaungOrderType: (vue.fuzhaungOrderTypes == 0)?null:vue.fuzhaungOrderTypes
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
