    ...
    <div class="page-order-comfirm">
        <div class="section-con">
            ...
            <div class="orderBox">
                <div class="orderC-table">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td class="text-left">
                                <div class="order-gw clearfix">
                                    <div class="order-img"><img :src="item.fuzhaungPhoto"/></div>
                                    <div class="order-sum">
                                        <h2>{{item.fuzhaungName}}</h2>
                                    </div>
                                </div>
                            </td>
                            <td>{{item.fuzhaungNewMoney}} RMB</td>
                            <td>
                                {{item.buyNumber}}
                            </td>
                            <td >
                                <span class="priceD">￥{{(item.fuzhaungNewMoney*item.buyNumber).toFixed(2)}}</span>
                            </td>
                        </tr>
                        </tbody>
                    </table>

                    <div class="orderC-total" v-if="this.fuzhaungOrderPaymentTypes == 1">
                        <div class="order-t">
                            <dl class="clearfix">
                    ...
                    <div class="orderC-total" v-if="this.fuzhaungOrderPaymentTypes == 2">
                        <div class="order-t">
                            <dl class="clearfix">
                                <dd class="dl_ddL">总&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;价：</dd>
		...
		<script>
				...
				computed: {
					totalMoney: function() {
						...
						for (var item of this.dataList) {
							total += item.fuzhaungNewMoney * item.buyNumber
						}
						return total;
					}
				},
				methods: {
					...
					,payClick() {
						...
						let data={
                            ...
                            fuzhaungs:localStorage.getItem('fuzhaungs'),
                            yonghuId: localStorage.getItem('userid'),
                            fuzhaungOrderPaymentTypes: vue.fuzhaungOrderPaymentTypes,
						}
                        // 获取商品详情信息
                        layui.http.request(`fuzhaungOrder/order`, 'POST', data, (res) => {
                            // 订单编号
                            var orderId = genTradeNo();
							if(res.code == 0){
                                alert("下单成功，点击确定后跳转 我的订单页面");
                                window.location.href='../fuzhaungOrder/list.jsp';
							}
                       		 window.location.href='../fuzhaungOrder/list.jsp';
					    });
					}
			...
			layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload'], function() {
                ...
                // 获取服装购买的清单列表
				var fuzhaungs = localStorage.getItem('fuzhaungs');
				// 转换成json类型，localstorage保存的是string数据
				vue.dataList = JSON.parse(fuzhaungs);
